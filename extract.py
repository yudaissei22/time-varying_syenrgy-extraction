...
def example():
    ...
    synergies, (amplitude, delays) = generate_example_data(N, M, T, K, D, S, plot=False)

    dataset = np.loadtxt("nlopt/Aug--7-17-21-37-2023-motion.taulist")

    
    # for i in range(10):
    #     print("dataset has", dataset[i].shape)
...
def generate_example_data(N=3, M=3, T=100, K=3, D=4, S=15, plot=True):
    ...
    #     print("data shape is", data[n].shape)
    #     for k in range(K):
    ...
    #     dataset.append(data)
    #     print("data has shape", dataset[n].shape)

    tau_t = np.loadtxt("nlopt/Aug--7-17-21-37-2023-motion.taulist")
    dataset = np.delete(tau_t, 0, axis=1)
    ...
    if plot:
        ...
        for n in range(N):
            ...
            for m, ax in enumerate(axes):
                ...
                ax.plot(np.arange(data.shape[1]), data[n,m], "--", lw=2)
    ...
    return synergies, (amplitude, delays)
